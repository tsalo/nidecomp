...
def _godec(data, thresh=.03, rank=2, power=1, tol=1e-3, max_iter=100,
...
def _wrap_godec(data, rank=2, power=2, tol=1e-3,
                thresh=10, max_iter=2000, norm_mode='vn', random_seed=0,
                verbose=True):
    """
    ...
    # GoDec
    L, S, G = _godec(data_norm, thresh=thresh, rank=rank,
...
def godec(optcom_ts, mmix, mask, comptable, ref_img, ranks=[2],
          thresh=10, norm_mode='vn', power=2, max_iter=2000, out_dir='.'):
    """
    ...
    for rank in ranks:
        L, S, G = _wrap_godec(medn_ts, rank=rank, power=power, thresh=thresh,
                              max_iter=max_iter, norm_mode=norm_mode)
